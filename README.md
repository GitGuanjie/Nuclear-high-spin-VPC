...
A laptop-friendly quick reproduction of the principal workflows can then be launched with
...
the polar coherent spin state

\[
|CSS_z\rangle=|I,I\rangle
\]

is used as the primary TACT benchmark.

TACT dynamics initialized from both the equatorial coherent spin state and the polar \(CSS_z\) state are included for comparison.

The Python implementation independently gives the following first minima of the Kitagawa–Ueda squeezing parameter for TACT initialized from \(CSS_z\):
...
The comparison reveals a **speed–depth tradeoff** rather than a universal advantage of either protocol:
...
This reproduces representative TACT and optimized VPC results for both Kitagawa–Ueda and Wineland squeezing.
...
### Fig. 2 — VPC, OAT, and TACT comparison
...
- TACT with the equatorial coherent spin state,
- TACT with \(CSS_z\).
...
The comparisons use the nominal dimensionless normalization
...
A compact interactive reproduction of the TACT benchmark is available in
